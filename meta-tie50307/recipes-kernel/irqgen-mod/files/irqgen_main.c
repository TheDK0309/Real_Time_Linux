...
static int irqgen_probe(struct platform_device *pdev)
{
    ...
    spin_lock_init(&irqgen_data->irqgen_spinlock);
 ...
 err_sysfs_setup:
    printk(KERN_ERR KMSG_PFX "sysfs_setup() failed\n");
    irqgen_sysfs_cleanup(pdev);
    return retval;
 err:
    printk(KERN_ERR KMSG_PFX "probe() failed\n");
    iounmap(irqgen_reg_base);
    for (i=0; i<sizeof(&irqgen_data->intr_ids); ++i) {
        free_irq(irqgen_data->intr_ids[i], &pdev->dev);
    }
    devm_kfree(&pdev->dev, pdev);
...
static const struct of_device_id irqgen_dt_id[] = {

...
static irqreturn_t irqgen_irqhandler(int irq, void *data)
{
	...
	u32 regvalue = 0 | FIELD_PREP(IRQGEN_CTRL_REG_F_ACK,0b0000)
					 | FIELD_PREP(IRQGEN_CTRL_REG_F_HANDLED,0b1)
                     | FIELD_PREP(IRQGEN_CTRL_REG_F_ENABLE,0b1);
	iowrite32(regvalue,IRQGEN_CTRL_REG);
	//iowrite32(0b1111,IRQGEN_CTRL_REG_F_ACK);
    return IRQ_HANDLED;
...
void enable_irq_generator(void)
{
	...
	//iowrite32(0b1,IRQGEN_CTRL_REG_F_ENABLE);
}
...
void disable_irq_generator(void)
{
    ...
    // HINT: use iowrite32 and the bitfield macroes to modify the register fields
	u32 regvalue = 0
                   | FIELD_PREP(IRQGEN_GENIRQ_REG_F_AMOUNT,0x0000)
                   /*| FIELD_PREP(IRQGEN_GENIRQ_REG_F_DELAY,0b0000000000)
                   | FIELD_PREP(IRQGEN_GENIRQ_REG_F_LINE,0b0000)*/;
				   
	iowrite32(regvalue,IRQGEN_GENIRQ_REG);
	
	u32 regvalue1 = 0 /*| FIELD_PREP(IRQGEN_CTRL_REG_F_ACK,0b0000)*/
					 | FIELD_PREP(IRQGEN_CTRL_REG_F_HANDLED,0b0)
                     | FIELD_PREP(IRQGEN_CTRL_REG_F_ENABLE,0b0);
                   
	iowrite32(regvalue1, IRQGEN_CTRL_REG);
	//iowrite32(0x0000,IRQGEN_GENIRQ_REG_F_AMOUNT);
...
static int32_t __init irqgen_init(void)
{
    ...
    irqgen_reg_base = ioremap(IRQGEN_REG_PHYS_BASE,IRQGEN_REG_PHYS_SIZE);/*NULL;*/
    ...
    retval = _request_irq(IRQGEN_FIRST_IRQ,irqgen_irqhandler,IRQF_TRIGGER_RISING,"IRQ_HANDLER"/* FIXME: fill the first arguments */, NULL);
    if (retval != 0) {
